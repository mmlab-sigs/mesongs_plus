        ...
        with torch.no_grad():
            ...
            if train_context.checkpoint_dir is not None:
                if iteration in ppl.checkpoint_iterations:
                    ...
                    logger.info(f'Saving last checkpoint at iteration {iteration}, to ')
                    if isinstance(ppl.final_checkpoint, str):
                        logger.info(f'Saving last checkpoint at iteration {iteration}, to {ppl.final_checkpoint}')
                        gs_model.save(train_context.checkpoint_dir, iteration, type_=ppl.checkpoint_type)
                    else:
                        for type_ in ppl.final_checkpoint:
                            logger.info(f'Saving last checkpoint at iteration {iteration}, to {type_}')
