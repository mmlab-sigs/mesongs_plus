...
def evaluate(
    ...
    for idx, viewpoint in enumerate(cameras):
        ...
        if ppl_context is not None and ppl.save_rendered_image:
            torchvision.utils.save_image(rendered_image, ppl_context.render_result_dir / ('{0:05d}'.format(idx) + ".png"))
        ...
        cur_psnr = psnr_val.mean().double().item()
        cur_ssim = ssim_loss.mean().double().item()
        cur_lpips = lpips_val.mean().double().item()
        cur_l1 = l1_loss.mean().double().item()
    ...
    if ppl_context is not None and ppl_context.output_dir is not None:
        per_image_path = ppl_context.output_dir / "per_image_metrics.json"
        with open(per_image_path, "w") as f:
            json.dump(per_image_metrics, f, indent=2)
        logger.info(f"Per-image metrics saved to {per_image_path}")
