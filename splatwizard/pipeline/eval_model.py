...
def eval_model(ppl: PipelineParams, mp: ModelParams, opt: OptimizationParams, train_context):
    # 检查是否启用搜索（二分搜索优先于黄金分割搜索）
    enable_binary_search = getattr(mp, 'enable_binary_search', False)
    enable_golden_search = getattr(mp, 'enable_golden_search', False)
    need_extra_params = enable_binary_search or enable_golden_search
    ...
    decode_time = 0
    total_bytes = 0
    stage_timings = {}  # 细粒度 encode 子阶段计时
    if ppl.eval_mode == EvalMode.NORMAL:
        ...
        if ppl.save_bitstream:
            with open(train_context.base_output_dir / 'encoded.bin', 'wb') as f:
                #print("saving encoded.bin from ", train_context.base_output_dir / 'encoded.bin')
                with profile() as prof:
                    # 如果启用搜索（二分/三分），传递必要的参数
                    if need_extra_params:
                        gs_model.encode(f, scene=scene, ppl=ppl, opt=opt, mp=mp, init_model_fn=init_model, stage_timings=stage_timings)
                    else:
                        gs_model.encode(f, stage_timings=stage_timings)
            ...
            with tempfile.NamedTemporaryFile(mode='w+b', delete=True) as tmp_file:
                with profile() as prof:
                    # 如果启用搜索（二分/三分），传递必要的参数
                    if need_extra_params:
                        gs_model.encode(tmp_file, scene=scene, ppl=ppl, opt=opt, mp=mp, init_model_fn=init_model, stage_timings=stage_timings)
                    else:
                        gs_model.encode(tmp_file, stage_timings=stage_timings)
                encode_time = prof.duration
                # exit()
