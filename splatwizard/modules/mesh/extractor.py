...
def cull_scan_mesh(mesh, cam_iter, shrink_factor=1., masks=None):
        ...
        sampled_masks = []
        for i in tqdm(range(len(cams)), desc="Culling mesh given masks"):
            pose = pose_all[i]
            ...
            W = cams[i].orig_w.cuda() * shrink_factor
            H = cams[i].orig_h.cuda() * shrink_factor
        ...
        scale_mat = cams[0].scale_mat
        mesh.vertices = mesh.vertices * scale_mat[0, 0] + scale_mat[:3, 3][None]

        return trimesh_to_open3d(mesh)


def cull_scan_points(pcd, cam_iter: CameraIterator, shrink_factor=1., masks=None):
