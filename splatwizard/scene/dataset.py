...
class ViewDataset(Dataset):
    ...
    def load_cam(self, cam_info, image, resolution):
        ...
        if resized_image_rgb.shape[0] == 4:
            alpha = resized_image_rgb[3:4, ...]
            # Alpha-composite onto background color (matching readCamerasFromTransforms behavior).
            # Without this, transparent regions become black after mask_background multiplication
            # in Camera.set_image, while rendering uses white bg — causing a huge loss mismatch.
            bg = 1.0 if self.ppl.white_background else 0.0
            gt_image = gt_image * alpha + bg * (1.0 - alpha)
            # After compositing, do NOT pass the alpha mask to Camera.set_image,
            # otherwise it will re-multiply and destroy the composited background.
            loaded_mask = torch.Tensor([])
