...
from .config import ContextGSModelParams, ContextGSOptimizationParams
from splatwizard.modules.gaussian_model import GaussianModel
from splatwizard.modules.dataclass import RenderResult, LossPack, EvalPack
from splatwizard.metrics.loss_utils import l1_func, ssim_func

from splatwizard.utils.general_utils import (
    inverse_sigmoid, get_expon_lr_func, build_scaling_rotation,inverse_sigmoid, strip_symmetric
)


from compressai.entropy_models import EntropyBottleneck, GaussianConditional
from compressai.latent_codecs import LatentCodec, HyperLatentCodec
from .utils import Entropy_bernoulli, Entropy_gaussian, Entropy_factorized, \
                    STE_binary, STE_multistep, Quantize_anchor, \
                    anchor_round_digits, torch_unique_with_indices, \
                    encoder, decoder, \
...
class ContextGSModel(RenderMixin, LossMixin, DensificationAndPruneMixin, GaussianModel):
    ...
    def __init__(self, model_param: ContextGSModelParams):
        super().__init__()
        ...
        self.n_features_per_level = model_param.n_features

        self.resolutions_list = (18, 24, 33, 44, 59, 80, 108, 148, 201, 275, 376, 514)
        self.resolutions_list_2D = (130, 258, 514, 1026)
        self.ste_binary = True
        self.ste_multistep = True
        self.add_noise = False
        self.Q = 1
        self.use_2D = True
        self.decoded_version = True
        self.level_num = model_param.level_num
        self.adaptQ_per_channel = False
        self.hyper_divisor = model_param.hyper_divisor
        self.target_ratio = model_param.target_ratio
        self.level_scale = None
        self.disable_hyper = model_param.disable_hyper
        self.lmbda_rec = model_param.lmbda_rec
        self.lmbda = 0.001
        ...
        self.latent_codec = EntropyBottleneck(channels=self.feat_dim//self.hyper_divisor).cuda()
        ...
        for i in range(self.level_num):
            feat_out_dim = (self.feat_dim+6+3*self.n_offsets)*2+1+1+1 # feat grid offset
            context_dim = self.feat_dim + 6 + 3
            feat_in_dim = self.feat_dim//self.hyper_divisor+3 if i==self.level_num-1 else context_dim+self.feat_dim//self.hyper_divisor

            self.mlp_grid.append(nn.Sequential(
                nn.Linear(feat_in_dim, self.feat_dim*2),
                nn.ReLU(True),
                nn.Linear(self.feat_dim*2, feat_out_dim),
            ).cuda())
    ...
    def register_post_task(self, scheduler: Scheduler, ppl: PipelineParams, opt: ContextGSOptimizationParams):
        # Densification
        ...
        scheduler.register_task(range(opt.start_stat + 1, opt.update_until), task=self.training_statis_task)

        adjust_anchor_iters = [
            i for i in range(opt.update_from + 1, opt.update_until)
            if i % opt.update_interval == 0 and i not in range(3000, 4000)
        ]
        scheduler.register_task(adjust_anchor_iters, task=self.adjust_anchor)

        
        scheduler.register_task([opt.update_until], task=self.cleanup_task)
    ...
    def loss_func(self, viewpoint_cam, render_result: RenderResult, opt) -> (torch.Tensor, LossPack):
        ...
        loss = self.lmbda_rec*((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss) + 0.01 * scaling_reg

        if  render_result["bit_per_param"] is not None:
            loss = loss + self.lmbda * render_result["bit_per_param"]
    ...
    @task
    def adjust_anchor(self, opt:ContextGSOptimizationParams):
        # # adding anchors
    ...
    @task
    def densify_and_prune_task(self, opt: ContextGSOptimizationParams, step: int):
