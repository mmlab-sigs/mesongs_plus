...
class MesonGS(LossMixin, DensificationAndPruneMixin, GaussianModel):
    ...
    @torch.no_grad
    def encode(self, tmp_file: io.BufferedWriter, stage_timings: dict = None, **kwargs):
        # path = tmp_file.name
    ...
    def decode(self, tmp_file: io.BufferedReader):
        ...
        with tempfile.TemporaryDirectory() as exp_dir:
            ...
            q_orgb_i = torch.tensor(oef_vals["i"].astype(np.float32), dtype=torch.float, device="cuda").reshape(self.n_rfc - 3, -1).contiguous().transpose(0, 1)
