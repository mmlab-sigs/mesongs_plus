...
from .config import ChimeraGSParams, ChimeraGSOptimizationParams, ChimeraGSPruneOptimizationParams, Stage, \
...
class ChimeraGS(FlashGSRenderMixin, CompressRenderMixin, LossMixin, DensificationAndPruneMixin, GaussianModel):
    ...
    def __init__(self, model_param: ChimeraGSParams= None):
    ...
    def encode(self, path: io.BufferedWriter):
        ...
        # selection, xyz_bs1 = encode_anchor(self._xyz.detach().cpu().numpy() * (2 ** 20),
        #               pathlib.Path('/tmp_output/liuxiang/programs/mpeg-pcc-tmc13-master/build/tmc3/tmc3'))
        # print(anchor_bit)
        ...
        self.mask_prune(~mask)
        self._sort_morton()
        # selection, xyz_bs2 = encode_anchor(self._xyz.detach().cpu().numpy()* (2 ** 20),
        #                                    pathlib.Path(
        #                                        '/tmp_output/liuxiang/programs/mpeg-pcc-tmc13-master/build/tmc3/tmc3'))
        # self.mask_prune(selection)
    ...
    def decode(self, path: io.BufferedReader):
        ...
        # xyz = decode_coordinate(xyz_bs1, pathlib.Path('/tmp_output/liuxiang/programs/mpeg-pcc-tmc13-master/build/tmc3/tmc3'))
        # self._xyz = nn.Parameter(torch.from_numpy(xyz /  (2 ** 20),).to(self._opacity.device))
        ...
        # xyz = decode_coordinate(xyz_bs2, pathlib.Path('/tmp_output/liuxiang/programs/mpeg-pcc-tmc13-master/build/tmc3/tmc3'))
        # self._xyz = nn.Parameter(torch.from_numpy(xyz / (2 ** 20),).to(self._opacity.device))
