...
def vq_features(
    ...
    sh_keep_threshold: float = -1.0,
    sh_keep_topk: int = -1,
    quantize_kept: bool = True,
    kept_quant_bits: int = 8,
    kept_quant_blocks: int = 80,
):
    '''
    borrowed from c3dgs, check: https://arxiv.org/abs/2401.02436
    
    Args:
        sh_keep_threshold: importance threshold for keeping full SH sequences.
                          Points with raw importance > sh_keep_threshold will be 
                          kept as individual codebook entries (not quantized).
                          Uses raw importance values (not normalized), consistent with pruning logic.
                          Default -1.0 to disable this feature.
        sh_keep_topk: keep top-K most important points with full SH sequences.
                     If set (>0), overrides sh_keep_threshold.
                     Default -1 to disable this feature.
    '''
    importance_n = importance/importance.max()
    
    # Check if we need to separate high-importance points
    # Priority: topk > threshold
    if sh_keep_topk > 0:
        # Top-K mode: keep top-K most important points
        num_keep = min(sh_keep_topk, features.shape[0])
        _, top_indices = torch.topk(importance, k=num_keep, largest=True)
        keep_mask = torch.zeros(features.shape[0], dtype=torch.bool, device=features.device)
        keep_mask[top_indices] = True
        
        print(f"Keeping top-{num_keep}/{features.shape[0]} most important points with full SH")
        if num_keep > 0:
            kept_imp = importance[keep_mask]
            print(f"  Kept points importance: min={kept_imp.min().item():.4f}, max={kept_imp.max().item():.4f}, mean={kept_imp.mean().item():.4f}")
        print(f"  All points importance: min={importance.min().item():.4f}, max={importance.max().item():.4f}, mean={importance.mean().item():.4f}")
    elif sh_keep_threshold > 0:
        # Threshold mode: use raw importance value, consistent with pruning logic
        keep_mask = importance > sh_keep_threshold
        num_keep = keep_mask.sum().item()
        print(f"Keeping {num_keep}/{features.shape[0]} points with full SH (raw importance > {sh_keep_threshold})")
        print(f"  Importance stats: min={importance.min().item():.4f}, max={importance.max().item():.4f}, mean={importance.mean().item():.4f}")
    else:
        # No keep strategy enabled
        keep_mask = None
        num_keep = 0
    
    if keep_mask is not None and num_keep > 0:
        # Split features into keep and vq groups
        keep_features = features[keep_mask]
        
        # Quantize kept features if enabled
        if quantize_kept:
            print(f"Quantizing {num_keep} kept features with {kept_quant_bits}-bit, {kept_quant_blocks} blocks")
            keep_q_indices, keep_scales, keep_zero_points, keep_split = quantize_kept_sh(
                keep_features,
                n_block=kept_quant_blocks,
                num_bits=kept_quant_bits
            )
            
            # Calculate storage sizes
            keep_q_size = keep_q_indices.numel() * (1 if kept_quant_bits <= 8 else 2)
            keep_param_size = (keep_scales.numel() + keep_zero_points.numel()) * 4  # float32
            keep_original_size = keep_features.numel() * 4  # float32
            
            print(f"  Kept SH quantization:")
            print(f"    Original: {keep_original_size / 1024 / 1024:.2f} MB")
            print(f"    Quantized indices: {keep_q_size / 1024 / 1024:.2f} MB")
            print(f"    Scales+ZPs: {keep_param_size / 1024 / 1024:.2f} MB")
            print(f"    Total: {(keep_q_size + keep_param_size) / 1024 / 1024:.2f} MB")
            print(f"    Compression ratio: {keep_original_size / (keep_q_size + keep_param_size):.2f}x")
        else:
            keep_q_indices = None
            keep_scales = None
            keep_zero_points = None
            keep_split = None
        
        # Step 1: ALL points participate in VQ clustering (not just non-kept points)
        # This ensures the VQ codebook covers the entire feature space,
        # so when kept points are removed during eval, they can fall back to good VQ representations.
        ...
        vq_model.uniform_init(features)  # Initialize with ALL points
        
        errors = []
        for i in trange(steps, desc="VQ clustering (all points)"):
            batch_size = min(vq_chunk, features.shape[0])
        ...
        # Step 2: Get VQ indices for ALL points
        _, vq_indices_all = vq_model(features)
        vq_codebook = vq_model.codebook.data.detach()
        
        print(f"VQ codebook trained on ALL {features.shape[0]} points (codebook_size={vq_codebook.shape[0]})")
        
        # Step 3: Build final codebook = [VQ codebook, kept features]
        vq_cb_size = vq_codebook.shape[0]
        final_codebook = torch.cat([vq_codebook, keep_features], dim=0)

        # Step 4: Build indices
        #   - Non-kept points: use their VQ index (0 ~ vq_cb_size-1)
        #   - Kept points: use new index (vq_cb_size ~ vq_cb_size+num_keep-1), overriding their VQ index
        final_indices = vq_indices_all.clone()
        final_indices[keep_mask] = torch.arange(vq_cb_size, vq_cb_size + num_keep,
                                                 dtype=torch.long, device=features.device)

        print(f"Final codebook size: {final_codebook.shape[0]} (kept: {num_keep}, VQ: {vq_codebook.shape[0]})")

        # Step 5: Save VQ indices for ALL points (including kept points' original VQ assignments)
        # Used for dynamic adjustment during eval: when a kept point is removed, revert to its VQ index
        vq_indices_for_all = vq_indices_all  # Already contains VQ indices for ALL points

        print(f"Pure VQ codebook size: {vq_codebook.shape[0]} (for dynamic adjustment)")

        # Return quantized version if enabled
        return final_codebook, final_indices, {
            'keep_q_indices': keep_q_indices,
            'keep_scales': keep_scales,
            'keep_zero_points': keep_zero_points,
            'keep_split': keep_split,
            'num_keep': num_keep,
            'keep_mask': keep_mask,
            'pure_vq_codebook': vq_codebook,  # Pure VQ codebook (no kept points)
            'vq_indices_for_all': vq_indices_for_all,  # VQ indices for ALL points (naturally computed)
        }
    
    # Original VQ path (no threshold or threshold disabled)
    vq_model = VectorQuantize(
        channels=features.shape[-1],
        codebook_size=codebook_size,
        decay=decay,
    ...
    # Return without quantization metadata for backward compatibility
...
def quantize_kept_sh(
    ...
    Args:
        kept_features: [N_keep, channels] 保留点的 SH 特征（例如 [300000, 45]）
        n_block: 分块数量，默认 80
        num_bits: 量化位宽，8 或 16
    
    Returns:
        q_indices: 量化索引 [channels, N_keep] (int32)
        scales: 每块的 scale [channels, n_block]
        zero_points: 每块的 zero_point [channels, n_block]
        split: 分块长度列表
    """
    N_keep, channels = kept_features.shape
    
    # 计算分块大小
    split = split_length(N_keep, n_block)
    splits_tensor = torch.tensor(split, device=kept_features.device, dtype=torch.int32)
    splits_tensor = splits_tensor.unsqueeze(0).repeat(channels, 1)  # [channels, n_block]
    
    power_qbits = torch.full(
        (channels, n_block), 
        2**num_bits - 1, 
        device=kept_features.device, 
        dtype=torch.int32
    )
    
    # 使用分块量化函数
    q_indices, scales, zero_points, max_vals, min_vals = pure_quant_wo_minmax(
        kept_features,      # [N_keep, channels]
        splits_tensor,      # [channels, n_block]
        power_qbits         # [channels, n_block]
    )
    # q_indices: [channels, N_keep]
    # scales, zero_points: [channels, n_block]
    
    return q_indices, scales, zero_points, split


def dequantize_kept_sh(
    ...
    Args:
        q_indices: [channels, N_keep] 量化索引
        scales: [channels, n_block]
        zero_points: [channels, n_block]
        split: 分块长度列表
        num_bits: 量化位宽
    
    Returns:
        kept_features: [N_keep, channels] 反量化后的 SH 特征
    """
    channels, N_keep = q_indices.shape
    n_block = len(split)
    ...
    for b in range(n_block):
        length = split[b]
        block_q = q_indices[:, start:start+length]  # [channels, length]
        block_scale = scales[:, b:b+1]              # [channels, 1]
        block_zp = zero_points[:, b:b+1]            # [channels, 1]
        
        # 反量化: x = scale * (q - zero_point)
        block_deq = block_scale * (block_q.float() - block_zp)  # [channels, length]
        dequantized.append(block_deq)
        start += length
    
    # 拼接所有块
    dequantized = torch.cat(dequantized, dim=1)  # [channels, N_keep]
